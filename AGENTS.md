...
> This file is read by Codex and other AI agents at session start.
...
Set at `/genesis` in `vault_language:` field below. In Codex, the equivalent entrypoint is `$genesis`. All user-facing output uses this language.
...
| `/distill <source>` | `$distill <source>` | Cross-language cognitive reconstruction of foreign source |
| `/lint [--rebuild-graph]` | `$lint [--rebuild-graph]` | Health check; verify wiki/ontology consistency |
...
3. Note which workflow entrypoint the user's request maps to (`/genesis` in Claude Code, `$genesis` in Codex, etc.)
