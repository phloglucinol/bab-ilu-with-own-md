...
# Redact BASE_URL to domain only — security audit 2026-04-21. The full URL
# often encodes provider/tenant/proxy details, and the log file lives in
# /tmp which is world-readable on shared systems.
try:
    from urllib.parse import urlparse
    _base_domain = urlparse(BASE_URL).netloc if BASE_URL else "<unset>"
except Exception:
    _base_domain = "<parse-error>"
debug_log(f"BASE_URL domain: {_base_domain}")
        ...
        try:
            with httpx.Client(timeout=300.0) as client:
                ...
                if response.status_code != 200:
                    # Security: do NOT log response.text — API error bodies
                    # can echo authorization hints / proxy metadata. Return
                    # it only to the caller (who already has credentials).
                    error_msg = f"API error {response.status_code}: {response.text[:500]}"
                    debug_log(f"API error: status={response.status_code}")
