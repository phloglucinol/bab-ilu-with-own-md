#!/usr/bin/env python3
"""Wiki lint helper — structural checks for a Bab-ilu vault.
...
# Schema constants — single source of truth. See tools/_schemas.py for the spec; do not duplicate the definitions here.
try:
    from _schemas import (  # type: ignore[no-redef]
        ENTITY_DIRS,
        FIELD_DEFAULTS,
        REQUIRED_FIELDS,
        VALID_EDGE_TYPES,
...
except ImportError:  # allow tools/ to also be importable as a package (`python -m tools.lint`)
    from tools._schemas import (  # type: ignore[no-redef]
        ENTITY_DIRS,
        FIELD_DEFAULTS,
        REQUIRED_FIELDS,
        VALID_EDGE_TYPES,
...
# Lens-aware checks are opt-in via --lens. Imported lazily-compatible (the
# module exists in every v2.1 vault, so top-level import is safe).
try:
    from lens_loader import LensConfig, LensValidationError, load_lens
except ImportError:  # allow tools/ to also be importable as a package
...
def lint(wiki_dir: Path) -> list[LintIssue]:
    ...
    # K2 · Karpathy 原教旨 active-suggestion rules (PRD §0.5.4).
    # Stale and material-request are lens-agnostic and always run.
    # Contradiction needs a lens and is called from main() when --lens
    # is supplied — adding it here would need a lens arg on lint().
    try:
        from lint_rules import check_stale_claims, check_material_requests
    except ImportError:  # pragma: no cover — tools/ importable as package
        from tools.lint_rules import (  # type: ignore[no-redef]
            check_stale_claims, check_material_requests,
        )
    issues.extend(check_stale_claims(wiki_dir, pages))
    issues.extend(check_material_requests(wiki_dir, pages))
...
# ---------------------------------------------------------------------------
# A8 · Lens-aware checks (opt-in via --lens)
#
# These checks run ALONGSIDE the v2.0 entity checks when a lens id is supplied.
# They operate on every `.md` under wiki_dir (not just ENTITY_DIRS) because the
# tier-0 / tier-1-cluster directories for a given lens are not enumerated in
# _schemas.ENTITY_DIRS — each lens contributes its own conventions. Keeping
# the scan independent means adding a lens never forces a schema edit.
# ---------------------------------------------------------------------------


def _scan_all_markdown(wiki_dir: Path) -> dict[str, Path]:
    """Slug -> path for every `.md` under wiki_dir (recursive).

    `find_all_pages` stays bound to the v2.0 ENTITY_DIRS so existing
    check_* helpers keep their scope. Lens-aware checks need the broader
    view because tier dirs (aesthetic/, panel/, …) vary per lens.
    """
    pages: dict[str, Path] = {}
...
def _page_type(fpath: Path, wiki_dir: Path) -> str:
    """Frontmatter `type:` wins; fall back to the parent directory name.

    Parent-dir fallback mirrors how the v2.0 checks infer type from
    location (papers/foo.md → "papers"); lens conventions are expected
    to follow the same on-disk layout.
    """
    try:
...
def check_lens_anchors(
    wiki_dir: Path, pages: dict[str, Path], lens: LensConfig
) -> list[LintIssue]:
    """Tier-0 pages must populate at least one authority field from the lens.

    A lens without `anchors.authority_fields` (e.g. zettelkasten) yields
    an empty list — the absence of institutional IDs is intentional for
    those disciplines and should not manufacture warnings.
    """
    ...
    tier_0 = str(lens.entity_model.get("tier_0") or "")
    if not tier_0:
        return issues

    for slug, fpath in pages.items():
        page_type = _page_type(fpath, wiki_dir)
        if page_type != tier_0:
            continue
        try:
            fm = extract_frontmatter(fpath.read_text(encoding="utf-8"))
            ...
            continue
        if any(fm.get(field) for field in authority_fields):
            continue
        rel = str(fpath.relative_to(wiki_dir))
        issues.append(
            LintIssue(
                "🟡", "lens-anchor", rel,
                f"tier-0 '{slug}' has no authority anchor "
                f"(lens '{lens.id}' expects one of: "
                f"{', '.join(authority_fields)})",
                suggestion=(
                    "Populate one of: " + ", ".join(authority_fields)
                    + " — see schema.md for the authoritative-ID contract."
                ),
            )
        )
    return issues
...
def check_lens_tier_consistency(
    wiki_dir: Path, pages: dict[str, Path], lens: LensConfig
) -> list[LintIssue]:
    """Tier-1-cluster wikilinks must resolve to tier-0 or tier-1-atom pages.

    Broken targets are intentionally ignored here — `check_broken_links`
    already reports them, and double-flagging the same slug would create
    duplicate noise. Only existing targets with the wrong type are raised.
    """
    issues: list[LintIssue] = []
    tier_0 = str(lens.entity_model.get("tier_0") or "")
    tier_atom = str(lens.entity_model.get("tier_1_atom") or "")
    tier_cluster = str(lens.entity_model.get("tier_1_cluster") or "")
    if not (tier_0 and tier_cluster):
        return issues
    allowed = {tier_0, tier_atom} if tier_atom else {tier_0}

    # Compute types once up-front. Without this cache, every wikilink in every
    # cluster page triggers a fresh frontmatter read of the target, blowing up
    # to O(cluster_pages * links) disk reads on large vaults.
    types = {slug: _page_type(fpath, wiki_dir) for slug, fpath in pages.items()}

    for slug, fpath in pages.items():
        if types.get(slug) != tier_cluster:
            continue
        try:
            content = fpath.read_text(encoding="utf-8")
        except OSError:
        ...
        for m in WIKILINK_RE.finditer(content):
            ...
            if target_slug not in pages:
                continue  # broken link — handled elsewhere
            target_type = types.get(target_slug, "")
            ...
            issues.append(
                LintIssue(
                    "🟡", "lens-tier", rel,
                    f"tier-1-cluster '{slug}' links to '{target_slug}' "
                    f"of type '{target_type}' (lens '{lens.id}' expects "
                    f"{sorted(allowed)})",
                )
            )
    return issues
...
def rebuild_graph(
    ...
    lenses_base: Optional[Path],
) -> subprocess.CompletedProcess:
    """Invoke tools.graph_analyzer as a subprocess to refresh the graph.

    Delegating keeps the two tools decoupled: the analyzer owns its CLI
    contract (nodes.jsonl, edges.jsonl, clusters) and lint merely asks
    it to re-run against the same vault. The analyzer's --vault flag
    expects the vault root (containing wiki/); lint passes
    `wiki_dir.parent` to satisfy that contract.

    Invoked as `python -m tools.graph_analyzer ...` from the repo root.
    Both the `-m` form and the bare-script form now work (both forms are
    covered by tests/test_tools_invokable_as_scripts.py), but we keep
    `-m` here for two reasons: (a) it's how we invoke the module
    internally via run_ingest's own dispatcher, so the two paths stay
    consistent; (b) `-m` is immune to PYTHONPATH surprises when lint
    itself is invoked from an unusual cwd.
    """
    cmd: list[str] = [
        sys.executable, "-m", "tools.graph_analyzer",
        "--vault", str(wiki_dir.parent),
    ]
    if lens_id:
        cmd += ["--lens", lens_id]
    if lenses_base is not None:
        cmd += ["--lenses-base", str(lenses_base)]
    # Run from the repo root so `-m tools.graph_analyzer` resolves.
    repo_root = Path(__file__).resolve().parent.parent
    return subprocess.run(cmd, capture_output=True, text=True, cwd=str(repo_root))


def main():
    parser = argparse.ArgumentParser(description="Bab-ilu linter")
    ...
    parser.add_argument("--lens", default=None,
                        help="Lens id for lens-aware checks (anchor + tier consistency)")
    parser.add_argument("--lenses-base", default=None,
                        help="Override default .agent/lenses/ base (mainly for tests)")
    parser.add_argument("--rebuild-graph", action="store_true",
                        help="Delegate to tools/graph_analyzer.py after linting")
    ...
    # Lens-aware extension: append anchor + tier checks when a lens is
    # supplied. Failures to load the lens (missing id, schema violation)
    # are fatal — we refuse to silently fall back to v2.0-only mode
    # because the user explicitly asked for lens awareness.
    if args.lens:
        lenses_base = Path(args.lenses_base) if args.lenses_base else Path(".agent/lenses")
        try:
            lens = load_lens(args.lens, base=lenses_base)
        except FileNotFoundError as exc:
            print(f"Error: lens '{args.lens}' not found: {exc}", file=sys.stderr)
            sys.exit(1)
        except LensValidationError as exc:
            print(f"Error: lens '{args.lens}' failed validation: {exc}",
                  file=sys.stderr)
            sys.exit(1)
        # Lens-aware checks scan ALL markdown, not just ENTITY_DIRS — tier
        # directories (aesthetic/, panel/, patterns/, note/, …) vary per lens
        # and should not require schema edits to become lintable.
        lens_pages = _scan_all_markdown(wiki_dir)
        issues.extend(check_lens_anchors(wiki_dir, lens_pages, lens))
        issues.extend(check_lens_tier_consistency(wiki_dir, lens_pages, lens))
        # K2 Rule A: lens-scoped contradiction detection (authority fields).
        try:
            from lint_rules import check_contradictions
        except ImportError:  # pragma: no cover
            from tools.lint_rules import check_contradictions  # type: ignore[no-redef]
        issues.extend(check_contradictions(wiki_dir, lens_pages, lens))
    ...
    # --rebuild-graph delegates to graph_analyzer.py. Run AFTER lint so the
    # analyzer sees any --fix edits we just applied.
    if args.rebuild_graph:
        rg_lenses_base = Path(args.lenses_base) if args.lenses_base else None
        rg = rebuild_graph(wiki_dir, lens_id=args.lens, lenses_base=rg_lenses_base)
        if rg.returncode != 0:
            # Surface analyzer errors without clobbering lint output ordering.
            print(f"[rebuild-graph] graph_analyzer exited {rg.returncode}",
                  file=sys.stderr)
            if rg.stderr:
