...
def cmd_launch(cfg: dict, args: argparse.Namespace) -> None:
    ...
    # Build log file path — reject path traversal (security audit 2026-04-21).
    # shlex.quote blocks shell injection but does not block `../../outside`
    # escaping the work_dir once tee executes.
    log_file = args.log_file or f"logs/{session_name}.log"
    if re.search(r"(^|/)\.\.(/|$)", log_file) or log_file.startswith("/"):
        _error(
            f"log_file path must be relative to work_dir and free of "
            f"'..' segments, got: {log_file!r}"
        )
